 ...
 * @Description TODO
 * @Author: Q-WHai
 * @Date: Created in 10:51 2019/04/10
 */
public class NanjingStation implements IStation {
    ...
    private Map<TravelPorts, Integer> _ticketSurplus = new HashMap<TravelPorts, Integer>() {
        {
            put(new TravelPorts("南京", "上海虹桥"), 20);
            put(new TravelPorts("南京南", "上海虹桥"), 16);
            put(new TravelPorts("武汉", "长沙"), 7);
        }
    };
    ...
    public boolean booking(StationPort from, StationPort to) {
        ...
        int tp = _ticketSurplus.get(new TravelPorts(from.getName(), to.getName()));
        _ticketSurplus.replace(new TravelPorts(from.getName(), to.getName()), tp - 1);
